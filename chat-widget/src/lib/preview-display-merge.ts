...
import type { SuggestedQuestionChip, WidgetInitResponse, WidgetPreviewOverrides } from "../types";
...
export function mergePreviewInitResponse(
  ...
  const baseChips: SuggestedQuestionChip[] = Array.isArray(base.bot?.suggestedQuestionChips)
    ? (base.bot!.suggestedQuestionChips as SuggestedQuestionChip[])
    : [];
  const hasChipOverlay = hasOwnOverride(overrides, "suggestedQuestionChips");
  const chipOverlayList = hasChipOverlay ? overrides.suggestedQuestionChips : undefined;

  function pickOverlayChipForLabel(label: string, index: number): SuggestedQuestionChip | undefined {
    const arr = chipOverlayList;
    if (!Array.isArray(arr) || arr.length === 0) return undefined;
    const at = arr[index];
    if (at && typeof at.label === "string" && at.label.trim() === label.trim()) return at;
    return arr.find((c) => c && typeof c.label === "string" && c.label.trim() === label.trim());
  }

  const suggestedQuestionChips: SuggestedQuestionChip[] = suggestedQuestions.map((label, i) => {
    const found = baseChips.find((c) => c?.label === label);
    const sid = found?.suggestionId?.trim();
    const oc = hasChipOverlay ? pickOverlayChipForLabel(label, i) : undefined;
    const hideChipTextInChat =
      oc !== undefined ? oc.hideChipTextInChat === true : found?.hideChipTextInChat === true;
    return {
      label,
      ...(sid ? { suggestionId: sid } : {}),
      ...(hideChipTextInChat ? { hideChipTextInChat: true as const } : {}),
    };
  });

...
function hexFillWithAlpha(hex6: string, alpha01: number): string {
  const h = hex6.trim();
...
export interface ChatComposerProps {
  ...
  /** Shown while dictation records (replaces waveform row when levels unavailable). */
  speechDictationListeningLabel?: string;
  speechRecordingVoiceLabel?: string;
  speechRecordingDictationLabel?: string;
  speechTranscribingVoiceLabel?: string;
  speechTranscribingDictationLabel?: string;
...
export function ChatComposer({
  ...
  sendLabel = "Send",
  accentColor = "#6366f1",
  inputMaxLength: inputMaxLengthProp,
  ...
  speechRecordingVoiceLabel = "Recording voice message…",
  speechRecordingDictationLabel,
  speechTranscribingVoiceLabel = "Processing...",
  speechTranscribingDictationLabel = "Transcribing...",
  ...
  const inputMaxLen = inputMaxLengthProp ?? CHAT_COMPOSER_INPUT_MAX_LENGTH;
  const inputDisabled = inputDisabledProp ?? disabledLegacy;
  const sendDisabled = sendDisabledProp ?? disabledLegacy;
  const dictationRecordingLabel =
    (speechRecordingDictationLabel ?? "").trim() ||
    (speechDictationListeningLabel ?? "").trim() ||
    "Dictating…";
  ...
  const usePrimaryBorder = composerBorderColor === "primary";
  const scrollHintBtn = dark
    ? usePrimaryBorder
      ? "text-gray-400 hover:bg-gray-700/45 hover:text-gray-300 focus-visible:ring-gray-500/40 focus-visible:ring-offset-0 focus-visible:ring-offset-gray-800/50 disabled:pointer-events-none disabled:text-gray-500/65 disabled:opacity-55 disabled:hover:bg-transparent disabled:hover:text-gray-500/65"
    ...
    : usePrimaryBorder
      ? "text-gray-500 hover:bg-gray-100/85 hover:text-gray-600 focus-visible:ring-gray-400/50 focus-visible:ring-offset-0 focus-visible:ring-offset-white disabled:pointer-events-none disabled:text-gray-400 disabled:opacity-65 disabled:hover:bg-transparent disabled:hover:text-gray-400"
    ...
    switch (composerControlStyle) {
      ...
      case "brand": {
        ...
        return {
          ...
          style: { backgroundColor: acc, color: fg },
        };
      }
      default:
        return { className: darkChip };
  ...
  const sendEmptyVisual = !hasTextOrAttachments && !sendDisabled;
  const sendSubmitIdleBgClass =
    sendEmptyVisual && composerControlStyle === "default"
      ? dark
        ? "!border-gray-600 !bg-gray-900/80 text-gray-200 hover:!bg-gray-900/80 active:!bg-gray-900/80"
        : "!border-gray-300 !bg-gray-200/90 text-gray-700 hover:!bg-gray-200/90 active:!bg-gray-200/90"
      : sendEmptyVisual && composerControlStyle === "defaultDark"
        ? dark
          ? "!border-gray-500 !bg-gray-900/80 !text-white hover:!bg-gray-900/80 active:!bg-gray-900/80"
          : "!border-gray-500 !bg-gray-800 !text-white hover:!bg-gray-800 active:!bg-gray-800"
        : "";
  const accFill = (accentColor ?? "").trim() || "#6366f1";
  const sendSubmitStyle =
    sendEmptyVisual && composerControlStyle === "brand" && composerActionBtn.style
      ? { ...composerActionBtn.style, backgroundColor: hexFillWithAlpha(accFill, 0.42) }
      : composerActionBtn.style;
            ...
            {inSpeech ? (
              ...
              >
                {speechCaptureProcessing ? (
                  ...
                  <div className="flex min-h-[36px] min-w-0 w-full flex-1 items-center gap-2">
                    ...
                    <MicIcon className="h-[18px] w-[18px] shrink-0 opacity-85" strokeWidth={1.85} />
                    <span
                      className={cx(
                        "min-w-0 flex-1 text-[15px] leading-5 font-medium",
                        dark ? "text-slate-200" : "text-slate-800",
                      )}
                    >
                      {dictationRecordingLabel}
                    </span>
                  </div>
                ) : (
                  <div className="flex min-h-8 min-w-0 w-full flex-1 items-center gap-2">
                    ...
                    <VoiceWavesIcon
                      stretch
                      className="h-5 min-h-[20px] min-w-0 shrink-0 animate-pulse text-emerald-400"
                    />
                    <span
                      className={cx(
                        "min-w-0 flex-1 text-[15px] leading-5 font-medium",
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  onChange(raw.length > inputMaxLen ? raw.slice(0, inputMaxLen) : raw);
                }}
                  ...
                  {inSpeech ? (
                    ...
                    <>
                      ...
                      <button
                        ...
                        aria-disabled={!hasTextOrAttachments || sendDisabled}
                        className={cx(composerActionBtn.className, sendSubmitIdleBgClass, "group")}
                        style={sendSubmitStyle}
                        aria-label={sendLabel}
                        title={sendLabel}
                      >
                        <span className={composerIconHoverTarget}>

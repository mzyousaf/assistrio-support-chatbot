...
export type ChatSpeechInputMeta = {
  ...
  mimeType?: string;
  durationMs?: number;
  /** Raw audio size for voice messages (bytes). */
  audioSizeBytes?: number;
  /** Dictation: cumulative recorded session length for analytics (ms). */
  dictationDurationMs?: number;
  /** Dictation pipeline: browser SR vs Whisper mic dictation. */
...
export interface ChatUIStrings {
  ...
  /** Section title above transcript in voice detail (embedded). */

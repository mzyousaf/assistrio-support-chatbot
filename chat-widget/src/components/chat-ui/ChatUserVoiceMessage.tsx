...
/** Matches `h-2.5 w-2.5` (10px). */
const PLAYHEAD_NOB_W_PX = 10;
const PLAYHEAD_NOB_HALF_PX = 5;

/**
 * Left edge of the nob so its center tracks `p` without jumps.
 * (Old flush vs `left: p%` + `translate(-50%)` made tiny `p` snap far left.)
 */
function nobLeftEdgeFromProgress(p: number): { left: string; transform: string } {
  return {
    left: `clamp(0px, calc(${p * 100}% - ${PLAYHEAD_NOB_HALF_PX}px), calc(100% - ${PLAYHEAD_NOB_W_PX}px))`,
...
export function ChatUserVoiceMessage({
  ...
  const durationForPlayhead =
    wsDuration > 0 ? Math.max(wsDuration, displayDuration) : displayDuration;
  ...
  const wf =
    speechRecordingWaveStyle === "brand" && (accentColor ?? "").trim()
      ...
      : baseWf;
  const playheadPt = nobLeftEdgeFromProgress(playheadProgress);
  ...
  return (
    ...
    >
      ...
      <div
        className="assistrio-voice-wavesurfer relative min-h-[36px] min-w-0 flex-1 overflow-hidden rounded-sm p-1"
        aria-hidden={!isReady}

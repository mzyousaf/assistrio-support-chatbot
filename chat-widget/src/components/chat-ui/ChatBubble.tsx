...
/** Filled “like” silhouette (Material-style), distinct from Lucide’s outline thumb when unselected. */
function SolidThumbsUpGlyph({ className }: { className?: string }) {
  return (
...
/** Filled dislike silhouette (Material-style), paired with {@link SolidThumbsUpGlyph}. */
function SolidThumbsDownGlyph({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M15 3H6c-.83 0-1.54.5-1.84 1.22l-3.02 7.05c-.09.23-.14.47-.14.73V14c0 1.1.9 2 2 2h6.31l-.95 4.57-.03.32c0 .41.17.79.44 1.06L9.83 23l6.58-6.59c.37-.36.59-.86.59-1.41V5c0-1.1-.9-2-2-2zm4 0v12h4V3h-4z" />
    </svg>
  );
}
            ...
            >
              <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1 text-[12px] leading-none">
                ...
                {relativeSentLabel && showAssistantThumbs ? (
                  <span
                    className="h-[12px] w-px shrink-0 self-center rounded-full bg-current opacity-40 mt-[2px]"
                    aria-hidden
                  />
                ) : null}
                {showAssistantThumbs ? (
                  <div className="flex items-center gap-2" role="group" aria-label="Rate this reply">
                    <button
                      ...
                      className={cx(
                        "inline-flex h-[14px] w-[14px] shrink-0 items-center justify-center rounded p-0 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1",
                        dark ? "focus-visible:ring-offset-gray-900" : "focus-visible:ring-offset-white",
                        message.feedbackRating === "up"
                          ? dark
                            ? "text-gray-300 focus-visible:ring-gray-500/70"
                            : "text-gray-500 focus-visible:ring-gray-400/70"
                          : dark
                    ...
                    >
                      {message.feedbackRating === "up" ? (
                        <SolidThumbsUpGlyph className="block h-[13px] w-[13px] shrink-0" />
                      ) : (
                        <ThumbsUp className="block h-[13px] w-[13px] shrink-0" strokeWidth={2} fill="none" aria-hidden />
                      )}
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      {message.feedbackRating === "down" ? (
                        <SolidThumbsDownGlyph className="block h-[13px] w-[13px] shrink-0" />
                      ) : (
                        <ThumbsDown className="block h-[13px] w-[13px] shrink-0" strokeWidth={2} fill="none" aria-hidden />
                      )}
                    </button>
                  </div>
                ) : null}

...
import { MIN_VOICE_RECORDING_UPLOAD_BYTES } from "../../lib/transcriptionUploadFile";
import {
  SpeechRecordingEmptyError,
  SpeechRecordingTooShortError,
  stopRecorderAndTranscribe,
} from "../../lib/stopRecorderAndTranscribe";
...
import { AssistrioBrandingPaid } from "./AssistrioBrandingPaid";
...
export interface ChatProps {
  ...
  /** Show a centered spinner while loading conversation messages from the server. */
  ...
  // Composer
  /** Return `false` to abort send and keep the composer as-is (e.g. retry after network failure). */
  onSend: (
    message: string,
  ...
  /** Rich chips from init (label + optional KB id). When set, drives click behavior for scoped first reply. */
  ...
  /** Hide visible text on first-screen suggestion chips (still sends the same prompt / suggestion id). */
  ...
  strings?: Partial<{
    ...
    /** Section title above transcript in the voice detail panel. */
    ...
    speechMicPermissionRequired?: string;
...
export function Chat({
  ...
  const handleSend = useCallback(async () => {
    ...
    const rollup = whisperDictationRollupRef.current;
    const hasWhisperDictation = rollup.sessionCount > 0;
    const dictateInput: ChatSpeechInputMeta | undefined =
      !hasVoice && hasWhisperDictation && displayText.trim().length > 0
        ? {
            ...
            dictationDurationMs: rollup.totalRecordDurationMs,
            durationMs: rollup.totalRecordDurationMs,
            ...(rollup.totalAudioSizeBytes > 0 ? { audioSizeBytes: Math.round(rollup.totalAudioSizeBytes) } : {}),
            ...(rollup.lastAudioMime?.trim() ? { mimeType: rollup.lastAudioMime.trim() } : {}),
          }
        : undefined;
    ...
    if (attach?.mode === "voice" && hasVoice) {
      ...
      void onSend(displayText, attach, files.length ? files : undefined, undefined);
      setInput("");
      return;
    }
    ...
    if (dictateInput) {
      sendResult = onSend(displayText, dictateInput, files.length ? files : undefined, {
        dictationSessionCount: rollup.sessionCount,
      });
    } else {
  ...
  const handleSuggested = useCallback(
    (text: string, meta?: { suggestionId?: string }) => {
      if (composerReadOnly || conversationLoading || isSending) return;
      onSuggestedQuestion?.(text);
      whisperDictationRollupRef.current = emptyWhisperDictationRollup();
      setInput(text);
      onSend(text, undefined, undefined, meta?.suggestionId ? { suggestionId: meta.suggestionId } : undefined);
      setInput("");
    },
  ...
  const s = {
    ...
    voiceTranscriptHeading: "Transcript",
      ...
      (() => {
        ...
        return (
          <ChatVoiceMessageDetailScreen
            ...
            onBack={() => setAttachmentsScreen(null)}
            transcriptHeadingLabel={s.voiceTranscriptHeading}
            copyTranscriptLabel={s.copy}
            transcriptCopiedLabel={s.copied}
          />
        );
      })()
    ) : null;

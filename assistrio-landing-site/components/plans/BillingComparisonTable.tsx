...
import { Check, ChevronDown, Infinity, Info, X } from 'lucide-react';
...
} from '@/lib/plans/billingPlanComparisonCopy';
...
function ComparisonCellValue({ value, compact = false }: { value: string; compact?: boolean }) {
  ...
  if (normalized === 'Available on paid plans') {
    return (
      <span
        className="inline-flex items-center justify-center gap-1.5 rounded-full bg-teal-50/80 px-2 py-0.5 ring-1 ring-teal-100/80"
        aria-label="Available as add-on"
      >
        <Check size={iconSize} strokeWidth={2.5} className="shrink-0 text-teal-600" aria-hidden />
        <span className={cn(textClass, 'text-xs font-medium text-teal-800')}>Add-on</span>
      </span>
    );
  }

  if (normalized === 'Coming soon' || normalized === 'Add-on') {
    return (
      <span
        className="inline-flex items-center justify-center gap-1.5 rounded-full bg-teal-50/80 px-2 py-0.5 ring-1 ring-teal-100/80"
        aria-label="Available as add-on"

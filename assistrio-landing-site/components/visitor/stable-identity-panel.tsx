 ...
 */
export function StableIdentityPanel({ variant = "default" }: Props) {
  ...
  return (
    <Card className="border-[var(--border-default)] border-l-[3px] border-l-[var(--brand-teal)]/70 bg-gradient-to-br from-white via-white to-slate-50/50">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand-teal-dark)]">Stable identity</p>
          <h3
            className={
              variant === "compact"
          ...
          >
            Save this ID to reconnect anywhere
          </h3>
        </div>
      ...
      {queryParamRejected ? (
        <p className="mt-3 rounded-[var(--radius-lg)] border border-amber-200/90 bg-amber-50/80 px-3 py-2 text-sm text-amber-950">
          ...
          generated a new id. Paste a saved id (Reconnect on the trial page or homepage) to restore your previous context.
        </p>
      ) : null}
      ...
      <div className="mt-4 space-y-3 text-sm leading-relaxed text-[var(--foreground-muted)]">
        <p>
          ...
          <code className="rounded bg-slate-100 px-1 text-xs">platformVisitorId</code>) — the same string the API uses for
          trial ownership, quota, and showcase runtime demos. It is <strong className="font-medium text-slate-800">not</strong>{" "}
          your chat history; the widget creates chat sessions separately (
          <code className="rounded bg-slate-100 px-1 text-xs">chatVisitorId</code>).
        </p>
        <p>
          <strong className="font-medium text-slate-800">Reconnect on another device</strong> by opening a link that
          includes <code className="rounded bg-slate-100 px-1 text-xs">?platformVisitorId=…</code> with your saved id, or
          by pasting the id into your embed config later (same as backend &quot;reconnect&quot; contract).
        </p>
        <p>
          <strong className="font-medium text-slate-800">Treat it like a private access token.</strong> Anyone with this
          id can hit the same anonymous quota APIs (for example the usage summary). Domain allowlists control{" "}
          <em>where</em> runtime may run — not <em>who</em> owns the id.
        </p>
        <p>
          <strong className="font-medium text-slate-800">Reconnect</strong> replaces the stored id in this browser — you are
          switching which anonymous bucket the marketing site uses (trial create, quota, demos). Your production embed
          snippet carries its own <code className="rounded bg-slate-100 px-1 text-xs">platformVisitorId</code>; update the
          snippet if you intentionally change which id should own runtime for that bot.
        </p>
      </div>

      <div className="mt-5 rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-slate-50/90 p-4 shadow-[var(--shadow-xs)]">
        <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Current id</p>
        {status === "loading" ? (

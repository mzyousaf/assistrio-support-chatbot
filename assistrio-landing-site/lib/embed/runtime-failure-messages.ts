...
export function describeRuntimeInitErrorCode(errorCode: string | undefined): {
  ...
  const table: Record<string, { title: string; detail: string }> = {
    EMBED_DOMAIN_NOT_ALLOWED: {
      title: "This site is not allowlisted for the bot",
      detail:
        "Update the bot's allowed domains (trial) or showcase registration so this page's hostname matches. If the browser blocked the request entirely, fix API CORS first (see Runtime deployment callout).",
    },
    ...
    EMBED_NO_ALLOWLIST: {
      title: "Bot has no embed allowlist",
      detail: "Configure at least one allowed hostname or exact origin for this bot.",
    },
    TRIAL_PLATFORM_VISITOR_OWNER_MISMATCH: {
      title: "Wrong stable id for this trial bot",
      detail: "Use the same platformVisitorId as when the trial was created, or reconnect with the saved id.",
    },
    PLATFORM_VISITOR_NOT_IN_BOT_ALLOWLIST: {
      title: "Visitor / URL not registered for this showcase bot",
      detail: "Use register-website on the gallery detail page or fix the allowlist in the product admin.",
    },
    ...
    TRIAL_RUNTIME_PLATFORM_VISITOR_ID_INVALID: {
      title: "Invalid stable id for trial",
      detail:
        'Trial runtime cannot use the reserved "anonymous" sentinel — the snippet must include your real platformVisitorId.',
    },
    VISITOR_ID_REQUIRED: {
      title: "platformVisitorId required",
      detail: "Trial bots require platformVisitorId on init — include it in AssistrioChatConfig (same as trial creation).",
    },
    BOT_NOT_FOUND: {
    ...
    INVALID_ACCESS_KEY: {
      ...
      detail: "Use the access key from trial success or public bot detail — must match this bot.",
    },
    INVALID_SECRET_KEY: {
    ...
    RATE_LIMITED: {
      ...
      detail:
        "The API returned 429 — wait retryAfterSeconds if present, then retry. This is per client IP as the API sees it (TRUST_PROXY behind load balancers). Not the same as CORS (no response body) or domain 403.",
    },
  };

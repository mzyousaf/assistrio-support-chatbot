  ...
  useEffect(() => {
    ...
    apiFetch("/api/user/visitors")
      .then(async (res) => {
        ...
        const raw = (await res.json()) as Array<{ visitorId?: unknown } & Record<string, unknown>>;
  ...
  if (authLoading || !user) {
    return (
      <AdminShell
        ...
        subtitle="Anonymous site analytics identities (funnel events), most recent first."
      >
                  ...
                  {visitors.length === 0 && (
                    <tr>
                      <td className="px-4 py-6 text-gray-500 dark:text-gray-400" colSpan={6}>
                        No marketing visitors recorded yet.
                      </td>
                    </tr>
                  )}
                </>

...
export function buildInitialBotPayload(
  ...
  return {
    ...
    allowedOrigins: Array.isArray((bot as { allowedOrigins?: unknown }).allowedOrigins)
      ? ((bot as { allowedOrigins: unknown[] }).allowedOrigins as unknown[])
          .map((row) => {
            ...
            return { origin, ...(label ? { label } : {}), isActive };
          })

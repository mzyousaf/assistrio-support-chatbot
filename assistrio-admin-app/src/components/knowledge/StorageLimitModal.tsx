...
export function StorageLimitModal(props: {
  ...
  onClose: () => void;
  botId: string;
  knowledgeUsage?: AdminKnowledgeUsage | null;
  /** Extra detail from API (e.g. usage line from error body). */
  backendMessage?: string | null;
}) {
  const navigate = useNavigate();
  const overviewPath = `/bots/${props.botId}/knowledge/overview`;
  ...
  return (
    <Modal
      ...
      description="This bot can't accept more knowledge until you remove content from its knowledge base."
    ...
    >
      <div className="space-y-4 text-sm text-slate-700">
        <ul className="m-0 list-disc space-y-1.5 pl-5 leading-relaxed text-slate-600">
          ...
          <li>Remove unused documents, FAQs, or notes to free space.</li>
        </ul>
...
export function StorageLowWarningModal(props: {
  ...
  const navigate = useNavigate();
  const overviewPath = props.botId ? `/bots/${props.botId}/knowledge/overview` : null;
    ...
    >
      <div className="space-y-4">
        <ul className="m-0 list-disc space-y-1.5 pl-5 text-sm leading-relaxed text-slate-600">
          <li>Free up items from Knowledge → Overview before uploading or importing more.</li>
...
export function KnowledgePlanLimitDetailActions({
  ...
  const navigate = useNavigate();
  const overviewPath = `/bots/${botId}/knowledge/overview`;

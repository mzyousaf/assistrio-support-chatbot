...
const STATUS_ROWS: { status: string; meaning: string }[] = [
  ...
  { status: 'Extracting', meaning: 'We are reading the text from the file so the bot can learn it.' },
  {
    status: 'Training Required',
    meaning: 'The text is ready, but the bot has not been told to learn it yet. Common when Auto Train is off—you can queue training yourself.',
  },
  {
    status: 'Training Queued',
    meaning: 'Learning this item is on the list. You may see a short countdown until it starts.',
  },
  { status: 'Training', meaning: 'The bot is actively learning this item right now.' },
  {
    status: 'Trained',
    meaning: 'This item is up to date in the bot’s memory. If “Use in replies” is on, it can help answer visitors.',
  },
  {
    status: 'Failed',
...
const FLOW_BULLETS: string[] = [
  'For files: upload first, then we pull out the text, then the bot learns it—either automatically (Auto Train) or when you choose to train.',
  'For Q&A, snippets, datasheets, and suggestions: when you save changes, the bot will learn them the same way—on your schedule or automatically.',
  '“Use in replies” only turns an item on or off for answers. It does not change how suggestion chips look in the chat widget.',
  'Storage numbers update when your content is saved. The “trained” step updates when the bot has finished learning the latest version.',
];
...
export function KnowledgeTrainingStatusesModal(props: { open: boolean; onClose: () => void }) {
  return (
    <Modal
      ...
      description="Simple guide to the words you see on knowledge items and in the training status card."
      size="lg"

...
import {
  tryHandleAdminResourceGone,
} from '@/lib/adminResourceUnavailable';
...
function workspaceDocumentUpdatedIso(d: AdminWorkspaceDocument | null): string | undefined {
...
function documentDetailCanDelete(d: AdminWorkspaceDocument | null): boolean {
  if (!d) return false;
...
export function DocumentDetailPage() {
  ...
  async function toggleUseInReplies(next: boolean) {
    ...
    try {
      const res = await patchAdminKnowledgeItemUseInReplies(botId, detailStatusItemId, { useInReplies: next });
      if (!res.ok) {
  ...
  async function confirmRemove() {
    ...
    setDeleting(true);
    try {
      const res = await deleteAdminBotDocument(botId, docId);
      if (tryHandleAdminResourceGone(navigate, res, `${base}/documents`)) {
        return;
      }

...
export function QaListPage() {
  ...
  async function downloadFaqSampleCsv() {
    if (!botId) return;
    const res = await getAdminKnowledgeFaqImportCsvSample(botId);
    if (!res.ok) {
  ...
  async function importFaqCsv(file: File | undefined) {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await postAdminKnowledgeFaqImportCsv(botId, fd);
      if (!res.ok) {
        const details =
  ...
  async function persistFaqForAdd(nextRow: QaRow, successToast: string): Promise<boolean> {
    ...
    try {
      ...
      const res = await postAdminKnowledgeFaq(botId, appendPayload as Record<string, unknown>);
      if (!res.ok) {

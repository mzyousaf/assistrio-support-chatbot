...
import type {
  AdminAgentTrainingDataSourceRow,
...
function DataSourceTrainingCounts({ row }: { row: AdminAgentTrainingDataSourceRow | undefined }) {
...
export function KnowledgeOverviewPage() {
  ...
  const [data, setData] = useState<AdminKnowledgeOverviewResponse | null>(() =>
  ...
  const load = useCallback(async () => {
    if (!botId) return;
    setLoadError(null);
    const res = await getAdminBotKnowledgeOverview(botId);
    if (!res.ok) {
  ...
  const confirmEnableAutoTrain = async () => {
    ...
    try {
      const retrainRes = await postAdminBotRetrainAgent(botId, { includeFailed: true });
      if (!retrainRes.ok) {
      ...
      const patchRes = await patchAdminBotKnowledgeTrainingSettings(botId, { autoTrainEnabled: true });
      if (!patchRes.ok) {
  ...
  const confirmDisableAutoTrain = async () => {
    ...
    setSettingsBusy(true);
    try {
      const patchRes = await patchAdminBotKnowledgeTrainingSettings(botId, { autoTrainEnabled: false });
      if (!patchRes.ok) {
  ...
  const onTrainOrRetrain = useCallback(async () => {
    ...
    setTrainBusy(true);
    try {
      const res = await postAdminBotRetrainAgent(botId, { includeFailed: true });
      if (!res.ok) {
  ...
  const saveReplyPrioritySettings = useCallback(
    async (
      ...
      try {
        const res = await patchAdminBotKnowledgeReplyPriority(botId, {
  ...
  const dataSourceTrainingByKey = useMemo(() => {
    const m = new Map<KnowledgeSourceBucket, AdminAgentTrainingDataSourceRow>();
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2">
                    ...
                    <div className="flex shrink-0 items-center gap-2.5">
                      ...
                      <Switch
                        checked={Boolean(tr.autoTrainEnabled)}
                  ...
                  <p className={styles.workspaceEditorSectionDescription}>
                    {tr.autoTrainEnabled
                      ...
                      : 'Changes stay pending until you queue training — use Retrain bot in the sidebar when you’re ready.'}{' '}
                    Retrain bot only adjusts scheduling; use Retry on a failed row when you need a fresh extract or train attempt.
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            >
              {knowledgeSourceRowsFromOverview(data).map((r) => (
                <li key={r.bucket}>
                  <Link
                    to={`/bots/${botId}/knowledge/${playgroundSegmentForKnowledgeBucket(r.bucket)}`}
          ...
          <ul className="m-0 list-none space-y-2 p-0 text-sm leading-relaxed text-slate-600">
            <li className="flex gap-2">
              ...
              <span>
                ...
                <strong className="font-medium text-slate-700">Retrain bot</strong>.
              </span>
            </li>
            <li className="flex gap-2">
              <span className="text-teal-600 tabular-nums" aria-hidden>
      ...
      <Modal
        ...
        description={
          <span className="text-sm leading-relaxed text-slate-600">
            New knowledge edits will stay pending until you use <strong>Retrain bot</strong> manually.
          </span>
        }
        footer={
          <div className="flex flex-wrap justify-end gap-2">

...
import type { AdminKnowledgeStatusItem, AdminWorkspaceDocument } from '../../../api/types';
...
export function DocumentEditPage() {
  ...
  const [doc, setDoc] = useState<AdminWorkspaceDocument | null>(null);
  ...
  const [kbPollItem, setKbPollItem] = useState<AdminKnowledgeStatusItem | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    setLoadError(null);
    const res = await getAdminBotDocument(botId, docId);
    if (!res.ok) {
      if (tryHandleAdminResourceGone(navigate, res, `${base}/documents`)) {
        setLoading(false);
        return;
      }
  ...
  async function confirmUpdateChanges() {
    ...
    setSaving(true);
    try {
      const res = await patchAdminBotDocument(botId, docId, {
        text: tx,
        title: displayTitle,
      });
      if (tryHandleAdminResourceGone(navigate, res, `${base}/documents`)) {
      ...
      appToast.success(
        autoTrainEnabled === true
          ...
          : autoTrainEnabled === false
            ? 'Document updated. Run Retrain bot from Knowledge Overview when you want this version in replies.'
            : 'Document updated.',
      );
      setSaveConflictNotice(null);
      setUpdateConfirmOpen(false);
            ...
            title={
              autoTrainEnabled === null
                ...
                : autoTrainEnabled
                  ...
                  : 'Save edits. Run Retrain bot from Knowledge Overview when you want replies to match this.'
            }
          >
            Update Changes
          </Button>
        ...
        description={
          autoTrainEnabled === null
            ...
            : autoTrainEnabled
              ...
              : 'This updates the saved document text. Run Retrain bot when you want replies to match.'
        }
        bodyNote={
          autoTrainEnabled === null ? (
            <p className={cn(styles.workspaceEditorHelperText, 'm-0')}>
              On Knowledge Overview, see whether this save queues training—or use{' '}
              <span className="font-medium text-slate-700">Retrain bot</span> if replies should update now.
            </p>
          ) : autoTrainEnabled ? (
            <p className={cn(styles.workspaceEditorHelperText, 'm-0')}>
            ...
            <p className={cn(styles.workspaceEditorHelperText, 'm-0')}>
              ...
              <span className="font-medium text-slate-700">Retrain bot</span> on Knowledge Overview.
            </p>
          )
        }

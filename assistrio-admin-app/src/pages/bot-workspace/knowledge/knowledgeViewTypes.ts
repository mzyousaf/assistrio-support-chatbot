import type {
  AdminBotWorkspaceBot,
  AdminKnowledgeStatusItem,
...
export type DocumentRowWithKbMeta = AdminWorkspaceDocument & {
  knowledgeItemId?: string;
...
export function documentRowContentExtractedForDisplay(doc: AdminWorkspaceDocument): boolean {
...
export function documentTrainingUiPhase(doc: AdminWorkspaceDocument): DocumentTrainingUiPhase {
...
export function documentPipelineStatusPillHoverDescription(doc: AdminWorkspaceDocument): string | null {
...
export function coerceDocumentPipelineStage(doc: AdminWorkspaceDocument): (typeof DOCUMENT_PIPELINE_STAGE_CODES)[number] | null {
...
export function workspaceDocumentPrimaryName(doc: AdminWorkspaceDocument): string {
...
export function documentRowLastTrainedIso(row: AdminWorkspaceDocument | DocumentRowWithKbMeta): string | null {
...
export function formatDocumentCharacterCountSizeLabel(doc: AdminWorkspaceDocument): string {
...
export function documentKnowledgeItemStoredUtf8Bytes(doc: Pick<AdminWorkspaceDocument, 'text'> | null): number | null {
  if (!doc) return null;
...
export function datasheetsFromBot(bot: AdminBotWorkspaceBot | null): TableBlock[] {
  if (!bot) return [];
  const b = bot as AdminBotWorkspaceBot & { knowledgeDatasheets?: TableBlock[]; knowledgeTables?: TableBlock[] };
...
export function snippetsFromBot(bot: AdminBotWorkspaceBot | null): SnippetRow[] {
  if (!bot) return [];
  const b = bot as AdminBotWorkspaceBot & { knowledgeSnippets?: SnippetRow[] };
  if (Array.isArray(b.knowledgeSnippets) && b.knowledgeSnippets.length > 0) {
    return b.knowledgeSnippets
      .map((s) => {
        ...
        const titleRaw = String(sug.title ?? ext.label ?? '').trim();
        const bodyRaw = String(sug.snippet ?? ext.description ?? '').trim();
...
export function faqsFromBot(bot: AdminBotWorkspaceBot | null): QaRow[] {
...
function dedupeKbPollDocumentsForMerge(poll: AdminKnowledgeStatusItem[]): AdminKnowledgeStatusItem[] {
  const byKb = new Map<string, AdminKnowledgeStatusItem>();
  for (const it of poll) {
...
export function mergeDocumentRowsWithKbStatusPoll(
  ...
  const byDocId = new Map<string, AdminKnowledgeStatusItem>();
  const byKbItemId = new Map<string, AdminKnowledgeStatusItem>();
  ...
  const merged = base.map((row) => {
    const kid = typeof row.knowledgeItemId === 'string' ? row.knowledgeItemId.trim() : '';
    let u: AdminKnowledgeStatusItem | undefined =

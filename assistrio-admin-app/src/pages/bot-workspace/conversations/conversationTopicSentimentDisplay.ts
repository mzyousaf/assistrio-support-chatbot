import type { AdminConversationMessage, AdminSentimentLabelId, AdminTopicsAnalyticsTopicId } from '@/api/types';
...
export function topicTaxonomyCustomerLabel(rawId: string | undefined | null): string {
  const t = (rawId ?? '').trim();
  if (!t) return '';
  const k = t as AdminTopicsAnalyticsTopicId;
...
export function sentimentTaxonomyCustomerLabel(raw: string | undefined | null): string {
  const k = (raw ?? '').trim().toLowerCase() as AdminSentimentLabelId;
...
export function messageShouldShowTopicSentimentTags(message: AdminConversationMessage): boolean {

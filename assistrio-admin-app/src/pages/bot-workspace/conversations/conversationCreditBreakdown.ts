import type { AdminConversationMessage, AdminConversationMessageCreditBreakdownRow } from '@/api/types';
...
function parseBreakdown(raw: AdminConversationMessage['creditBreakdown']): AdminConversationMessageCreditBreakdownRow[] | undefined {
  if (!Array.isArray(raw) || raw.length === 0) return undefined;
  const out: AdminConversationMessageCreditBreakdownRow[] = [];
...
function accumulateUsageModalitiesForSidebar(
  userRows: AdminConversationMessage[],
...
export function buildConversationCreditBreakdown(messages: AdminConversationMessage[] | null | undefined): ConversationCreditBreakdownPayload {

...
import { adminDocumentDeleteResolved } from '@/api/adminDeleteResult';
...
function docId(row: AdminWorkspaceDocument): string {
  const id = row._id;
...
function workspaceDocumentListRecencyMs(row: AdminWorkspaceDocument): number {
  const id = docId(row);
...
function documentLastTrainedIso(row: Record<string, unknown>): string | null {
  if (baselineDocumentRowTrainingStatus(row as AdminWorkspaceDocument) !== 'ready') return null;
...
function documentUploadLifecycleStatus(row: AdminWorkspaceDocument): string {
  return String((row as { uploadStatus?: unknown }).uploadStatus ?? (row as { documentStatus?: unknown }).documentStatus ?? '')
    .trim()
    .toLowerCase();
}

function isUploadedDocumentForTraining(row: AdminWorkspaceDocument): boolean {
...
function isDocumentRowUploadInProgress(row: AdminWorkspaceDocument, isClientPending: boolean): boolean {
...
function canShowDocumentRowDelete(row: AdminWorkspaceDocument, isPendingRow: boolean): boolean {
...
export function KnowledgeBaseSection() {
  const { bot, botId } = useAdminBotWorkspace();
  ...
  const kbBase = botId ? `/bots/${botId}/knowledge` : '';
  ...
  const knowledgePageMeta = {
    pageTitle: 'Documents',
    pageLead:
      'Long-form files your bot learns from. Upload below, or open one from the list. Extracted text is trained and used in replies when the file is on.',
  };
  ...
  const loadDocs = useCallback(async (opts?: { background?: boolean }) => {
    ...
    setDocErr(null);
    try {
      const res = await getAdminBotDocuments(wid, { page: 1, limit: 100 });
      if (!res.ok) {
        setDocErr(res.error);
      ...
      knowledgeDocumentsListCache.set(wid, {
        rows: serverDocs.map((d) => ({ ...(d as AdminWorkspaceDocument) })),
      ...
      setRows((prev) => {
        ...
        const serverIds = new Set(serverDocs.map((d) => docId(d as AdminWorkspaceDocument)));
        const pendingOnly = pending.filter((p) => !serverIds.has(docId(p)));
        const withMeta: DocumentRowWithKbMeta[] = serverDocs.map((d) => {
          const base = d as AdminWorkspaceDocument;
          const id = docId(base);
  ...
  const docStatusCounts = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      const up = documentUploadLifecycleStatus(r as AdminWorkspaceDocument);
      if (up === 'uploading') continue;
      if (up === 'upload_failed') {
        c.failed += 1;
        continue;
      }
      if (!isUploadedDocumentForTraining(r as AdminWorkspaceDocument)) continue;
      const cst = baselineDocumentRowTrainingStatus(r as AdminWorkspaceDocument);
    ...
    if (docStatusFilter !== 'all') {
      list = list.filter((r) => {
        ...
        if (docStatusFilter === 'failed') {
          if (up === 'upload_failed') return true;
        } else if (!isUploadedDocumentForTraining(r as AdminWorkspaceDocument)) {
          return false;
        }
        const cst = baselineDocumentRowTrainingStatus(r as AdminWorkspaceDocument);
  ...
  const docFilteredRows = useMemo(() => {
    ...
    if (q) {
      list = list.filter((r) => {
        ...
        const canonSt = baselineDocumentRowTrainingStatus(r as AdminWorkspaceDocument);
        const stLabel = documentTrainingStatusLabel(canonSt as Exclude<DocStatusFilter, 'all'>).toLowerCase();
        const pipeSt = coerceDocumentPipelineStage(r as AdminWorkspaceDocument) ?? '';
        const displayStatusLabel = formatDocumentTrainingStatusDisplayLabel(
          r as AdminWorkspaceDocument,
          documentDisplayOpts,
        ).toLowerCase();
    ...
    () =>
      docPaginatedRows
        .map((r) => {
          ...
          const docRow = r as AdminWorkspaceDocument;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const loadOverview = () => {
      void getAdminBotKnowledgeOverview(botId).then((res) => {
        if (cancelled || !res.ok) return;
        setDocumentDisplayOpts({
          autoTrainEnabled: Boolean(res.data.knowledgeTraining?.autoTrainEnabled),
        });
      });
    };
    loadOverview();
  ...
  async function removeDoc(documentIdToRemove: string): Promise<{ ok: true } | { ok: false; error: string }> {
    ...
    const res = await deleteAdminBotDocument(botId, documentIdToRemove);
    if (!adminDocumentDeleteResolved(res)) {
  ...
  async function uploadFilesFromPicker(files: File[]) {
    ...
    if (slotsRemaining <= 0) {
      appToast.error('Document limit reached', {
        description: `Each bot can have at most ${KNOWLEDGE_DOCUMENTS_MAX} documents. Remove one to upload more.`,
      });
      return;
    }
    ...
    try {
      for (let offset = 0; offset < toUpload.length; offset += KNOWLEDGE_DOCUMENT_UPLOAD_BATCH_MAX) {
        ...
        for (const file of chunk) {
          ...
          const pendingRow: AdminWorkspaceDocument = {
            _id: pendingId,
            title: file.name,
            status: 'uploading',
        ...
        let res: Awaited<ReturnType<typeof postAdminBotDocumentUpload>>;
        try {
          res = await postAdminBotDocumentUpload(botId, fd);
        } catch (e) {
  ...
  async function toggleDocIncluded(documentId: string, active: boolean) {
    ...
    const res = await patchAdminBotDocument(botId, documentId, { active });
    ...
    setRows((prev) =>
      prev.map((r) => (docId(r) === documentId ? { ...(r as AdminWorkspaceDocument), active } : r)),
    );
  ...
  async function bulkDeleteSelectedDocs() {
    ...
    setDocErr(null);
    const res = await postAdminBotDocumentsBulkDelete(botId, serverIds);
                  ...
                  <CardBody className="w-full min-w-0 px-0 py-0 sm:px-0 sm:py-0">
                    ...
                    >
                      ...
                      <p className="mt-2 w-full max-w-none px-1 text-xs leading-relaxed text-slate-500 sm:text-[0.8125rem]">
                        Up to {KNOWLEDGE_DOCUMENTS_MAX} documents per bot. Drag here, click to browse, or multi-select—up
                        ...
                        <table className="w-full min-w-[56rem] table-fixed border-collapse text-left text-sm">
                          ...
                          <tbody>
                            {docFilteredRows.length === 0 ? (
                              ...
                              const docRow = row as AdminWorkspaceDocument;

...
import { getAdminBotAgentTrainingStatus, getAdminBotKnowledgeStatus } from '@/api/adminApi';
import type {
...
import { redirectAdminWorkspacePollGone } from '@/lib/adminResourceUnavailable';
...
export type KbWorkspacePollingContextValue = {
  botId: string;
  trainingStatus: AdminAgentTrainingStatusResponse | null;
   ...
   */
  trainingStatusPeekRef: RefObject<AdminAgentTrainingStatusResponse | null>;
  ...
  applyAgentTrainingSnapshot: (s: AdminAgentTrainingStatusResponse | null) => void;
  registerKbPoll: (reg: KbPollRegistration) => () => void;
  /** Latest merged rows from typed `GET …/knowledge/status?type=…` polls. */
  knowledgeStatusItems: AdminKnowledgeStatusItem[] | null;
...
export function KbWorkspacePollingProvider({ botId, children }: { botId: string; children: ReactNode }) {
  ...
  const [trainingStatus, setTrainingStatus] = useState<AdminAgentTrainingStatusResponse | null>(null);
  const [trainingStatusError, setTrainingStatusError] = useState<string | null>(null);
  const [knowledgeStatusItems, setKnowledgeStatusItems] = useState<AdminKnowledgeStatusItem[] | null>(null);
  const [knowledgeStatusError, setKnowledgeStatusError] = useState<string | null>(null);
  const knowledgeStatusItemsRef = useRef<AdminKnowledgeStatusItem[] | null>(null);
  ...
  const trainingStatusPeekRef = useRef<AdminAgentTrainingStatusResponse | null>(null);

  /** Latest agent training snapshot (for coordinator tick training/status cadence). */
  const trainingStatusRef = useRef<AdminAgentTrainingStatusResponse | null>(null);
  trainingStatusRef.current = trainingStatus;

  const applyAgentTrainingSnapshot = useCallback((s: AdminAgentTrainingStatusResponse | null) => {
  ...
  const refreshTrainingStatus = useCallback(async () => {
    setTrainingStatusError(null);
    const res = await getAdminBotAgentTrainingStatus(botId);
    if (!res.ok) {
      if (redirectAdminWorkspacePollGone(navigate, res)) {
  ...
  const refreshKnowledgeStatus = useCallback(
    async (section?: KbTrainingAffectedType | 'all' | KbTrainingAffectedType[], itemId?: string) => {
      ...
      const results = await Promise.all(
        types.map((t) =>
          getAdminBotKnowledgeStatus(botId, {
            type: t,
      ...
      if (failed && !failed.ok) {
        if (redirectAdminWorkspacePollGone(navigate, failed)) {
          return;
        }
          ...
          tasks.push(
            (async () => {
              ...
              for (const req of dedup.values()) {
                const res = await getAdminBotKnowledgeStatus(botId, {
                  type: req.section,

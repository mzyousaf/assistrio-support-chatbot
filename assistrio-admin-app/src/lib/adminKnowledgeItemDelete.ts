...
import { postAdminBotKnowledgeItemsBulkDelete } from '@/api/adminApi';
 ...
 */
export function reportAdminKnowledgeItemDeleteRejected(
  ...
  if (gone && options.redirectListOnKbGone) {
    if (tryHandleAdminResourceGone(navigate, res, options.redirectListOnKbGone)) return true;
  }
  if (gone && !options.redirectListOnKbGone) {
...
export async function bulkDeleteAdminKnowledgeItems(
  botId: string,
  ...
  const res = await postAdminBotKnowledgeItemsBulkDelete(botId, [...ids]);

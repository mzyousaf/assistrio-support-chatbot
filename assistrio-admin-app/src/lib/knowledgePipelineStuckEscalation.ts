...
/** Maps {@link getKnowledgeItemDisplayStatus} keys → Admin KB status poll `displayStatus` strings. */
export function mapDisplayResultKeyToAdminApiStatus(key: KnowledgeItemDisplayResult['key']): string | null {
  switch (key) {
    case 'uploading':
      return 'uploading';
    case 'uploaded':
...
/** Admin `displayStatus` values where we allow removing an item even while the pipeline claims “busy”. */
...
export function buildDocumentStuckEscalationTimestamps(args: {
  kbPollItem: AdminKnowledgeStatusItem | null | undefined;
  doc: AdminWorkspaceDocument | null | undefined;
  ...
  if (!fromPoll && !fromDoc && doc) {
    ...
    inferred = mapDisplayResultKeyToAdminApiStatus(k) ?? '';
  }

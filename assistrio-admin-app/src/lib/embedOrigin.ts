...
/** Hosted customer app origin (share + iframe URLs). Prefer `VITE_CUSTOMER_APP_ORIGIN` in admin. */
export function getAdminAppPublicOrigin(): string {
  const env = (import.meta.env.VITE_CUSTOMER_APP_ORIGIN ?? import.meta.env.VITE_PUBLIC_APP_ORIGIN ?? '').replace(
    /\/$/,
    '',
  );
  if (env) return env;
...
/** @deprecated use getAdminAppPublicOrigin in admin app */
export function getCustomerAppPublicOrigin(): string {
  return getAdminAppPublicOrigin();
}

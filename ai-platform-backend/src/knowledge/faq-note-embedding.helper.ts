...
/** Widget suggestion chip + optional scoped facts (trainable when scoped is non-empty). */
export function buildSuggestionEmbeddingText(chipText: string, scopedInformation: string | undefined | null): string {
  const chip = (chipText ?? '').trim();
  const scoped = (scopedInformation ?? '').trim();

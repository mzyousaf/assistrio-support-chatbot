...
/**
 * Local file extraction for KB document uploads (after S3/URL has been downloaded to a temp path).
 * Supported extensions in {@link KbService.extractTextFromUpload}: `txt`, `md`/`markdown`, `pdf`, `docx`, `doc`.
 * URL and S3 sources are fetched upstream into a file path before this runs — not parsed here by URL string.
 * Unsupported types return `reason: 'unsupported_file_type'`. Oversized normalized text is truncated with
 * `reason: 'truncated'` (see `MAX_EXTRACTED_CHARS`).
 */
...
export class KbService {
  ...
  async extractTextFromUpload(params: {
    ...
    try {
      if (extension === 'txt') {
        rawText = await fs.readFile(params.filePath, 'utf8');

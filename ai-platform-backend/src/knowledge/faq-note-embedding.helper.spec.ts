...
describe('faq-note-embedding.helper', () => {
  ...
  describe('buildSuggestionEmbeddingText', () => {
    it('includes chip and scoped block', () => {
      expect(buildSuggestionEmbeddingText('Pricing', 'Plans start at $9.')).toBe(
        'Suggestion: Pricing\n\nScoped information:\nPlans start at $9.',
      );
    });
    it('allows empty scoped (ui-only metrics still use chip in metrics helper)', () => {
      expect(buildSuggestionEmbeddingText('Hello', '')).toBe('Suggestion: Hello\n\nScoped information:\n');
    });
  });

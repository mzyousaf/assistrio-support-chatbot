...
import type { MessageAnalyticsPayload } from './message-input-analytics.util';

export type ChatMode = 'user';

/** Subset of {@link ConversationOrigin} accepted on {@link RunChatInput}. */
export type ConversationOriginPayload = {
  source?:
    | 'script_embed'
    | 'iframe_embed'
    | 'shared_link'
...
/** Optional browser / coarse location hints from the client (never raw IP or raw UA). */
...
/**
 * Server-derived request metadata for analytics (IP used only to compute {@link hashIpForAnalytics}).
 */
...
export interface BotLike {
  _id: { toString(): string };
  /** Workspace tenant id (ObjectId string) when present on the bot document. */
  workspaceId?: string;
  /** Bot owner user id (ObjectId string) when present — used for ledger `customerId` fallback. */
  ownerId?: string;
  name?: string;
...
export interface RunChatInput {
  ...
  /**
   * Distinguish embed runtime vs admin widget preview vs share/iframe in stored conversations (default `runtime`).
   */
  ...
  /**
   * Stored on new conversations for hosted share / iframe attribution (optional).
   */
  ...
  /**
   * Knowledge base item id (`KnowledgeBaseItem` with `sourceType: suggestion`) when the visitor clicked a trainable suggestion chip.
   * First user turn only: scopes retrieval to that suggestion's chunks (no full KB).
   */
  suggestionId?: string;
  /** Optional coarse location / device hints (sanitized server-side). */
  analyticsContext?: AnalyticsContextPayload;
  /** Request IP (hashed only) + headers for server-side device inference. */
  analyticsRequestMeta?: AnalyticsRequestMeta;
  /** Optional client hints for how the user composed this message (validated server-side). */
  messageAnalytics?: MessageAnalyticsPayload;
}
...
export type RunChatResult =
  | {
    ...
    /** Persisted Mongo id for the assistant turn (widget feedback / workspace correlation). */

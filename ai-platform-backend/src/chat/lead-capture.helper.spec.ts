...
describe('extractLeadFieldsFromMessage', () => {
  ...
  it('extracts email from "my email is …" phrasing', () => {
    const r = extractLeadFieldsFromMessage('my email is zain@example.com', ['email'], fieldLabels);
    expect(r.extracted.email).toBe('zain@example.com');
  });
  ...
  it('extracts international phone when phrased as "call me at …"', () => {
    const r = extractLeadFieldsFromMessage('call me at +92 300 1234567', ['phone'], fieldLabels);
    expect(r.extracted.phone).toBeTruthy();
    expect(countDigits(r.extracted.phone!)).toBeGreaterThanOrEqual(10);
  });

  it('extracts name from "my name is X"', () => {
    const r = extractLeadFieldsFromMessage("My name is Alice", ['name'], fieldLabels);
    expect(r.extracted.name).toBe('Alice');
    expect(r.matchedByField.name).toBe('heuristic');
  });

  it('extracts name from "I am Zain" / "I\'m Sarah Khan" / "This is Ali" / "Call me Ahmed"', () => {
    expect(extractLeadFieldsFromMessage('I am Zain', ['name'], fieldLabels).extracted.name).toBe('Zain');
    expect(extractLeadFieldsFromMessage("I'm Sarah Khan", ['name'], fieldLabels).extracted.name).toBe('Sarah Khan');
    expect(extractLeadFieldsFromMessage('This is Ali', ['name'], fieldLabels).extracted.name).toBe('Ali');
    expect(extractLeadFieldsFromMessage('Call me Ahmed', ['name'], fieldLabels).extracted.name).toBe('Ahmed');
  });
  ...
  const nameNegatives = [
    ...
    'fine thanks',
    'I need help',
    'my problem is billing',
    'yes',
    'no',
    'later',
  ];
  it.each(nameNegatives)('does not capture name from %s', (msg) => {
    const r = extractLeadFieldsFromMessage(String(msg), ['name'], fieldLabels);
    expect(r.extracted.name).toBeUndefined();
  });

  it('captures "Zain here" only when name was the last asked field', () => {
    expect(extractLeadFieldsFromMessage('Zain here', ['name'], fieldLabels).extracted.name).toBeUndefined();
    expect(extractLeadFieldsFromMessage('Zain here', ['name'], fieldLabels, { lastAskedField: 'name' }).extracted.name).toBe(
      'Zain',
    );
  });
...
describe('detectDeclineResult / detectDecline', () => {
  ...
  it('treats standalone skip and maybe later as refusal / postpone', () => {
    expect(detectDeclineResult('skip')).toBe('declined');
    expect(detectDeclineResult('maybe later')).toBe('postponed');
  });
});

describe('messageProbablyRefusesLeadQuestion', () => {
  it('is true for refusal text without contact details', () => {
    expect(messageProbablyRefusesLeadQuestion("I don't want to share", detectDeclineResult("I don't want to share"))).toBe(
      true,
    );
  });

  it('is false when an email is present so other fields can still be captured', () => {
    const msg = 'no thanks, email me at x@y.com';
    expect(messageProbablyRefusesLeadQuestion(msg, detectDeclineResult(msg))).toBe(false);
  });
});
...
describe('mergeExtractedLeadData / mergeExtractedLeadDataWithDebug', () => {
  ...
  it('mergeExtractedLeadDataWithDebug returns overwritten and skipped', () => {
    ...
    expect(result.skipReasons?.email).toBe('rejected_low_confidence');
  });

  it('does not merge new field when confidence is explicitly low', () => {
    const out = mergeExtractedLeadData({}, { name: 'Zain' }, { name: 0.5 }, undefined, { name: 'text' });
    expect(out.name).toBeUndefined();
  });

  it('does not overwrite good existing name with low-confidence junk', () => {
    const out = mergeExtractedLeadData(
      { name: 'Zain' },
      { name: 'fine' },
      { name: 0.2 },

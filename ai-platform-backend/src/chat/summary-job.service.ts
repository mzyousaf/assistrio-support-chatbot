...
export class SummaryJobService {
  ...
  async enqueue(conversationId: Types.ObjectId, botId: Types.ObjectId): Promise<boolean> {
    ...
    const enqueueWindowSlot = Math.floor(Date.now() / SUMMARY_JOB_RECENT_MS);
    ...
    try {
      ...
      // Concurrent enqueue raced and another worker/request inserted the same live-window job.
  ...
  async processJob(
    ...
    const bot = await this.botModel.findById(job.botId).select('openaiApiKeyOverride active deletedAt').lean();
    if (botIsEffectivelyDeleted(bot as { active?: boolean; deletedAt?: Date | null })) {
      await this.markFailed(job, 'kb_bot_deleted');
      return;
    }
    const apiKey =

...
export function formatPromptFromContext(ctx: ChatKnowledgeContext): {
  ...
  if (ctx.suggestionScopeOnly) {
    userParts.push('--- Selected suggestion context (this turn only) ---');
    if (hasEvidence) {
      userParts.push(
        'Answer using only the following context. Do not use the full knowledge base. If the selected suggestion context is insufficient, say you do not have enough information for this suggestion.',
      );
      ...
      userParts.push(
        'No retrievable context is available for this suggestion. Do not use the full knowledge base. If you cannot answer from the conversation alone, say you do not have enough information for this suggestion.',
      );
    }
  } else if (hasEvidence) {

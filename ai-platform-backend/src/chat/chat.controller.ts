...
import type { AnalyticsContextPayload, BotLike, ConversationOriginPayload } from './chat-engine.types';
...
import { parseConversationOriginFromUnknown } from './conversation-analytics-origin.util';
import {
  ...
  async message(
    ...
    if (typeof req.isMultipart === 'function' && req.isMultipart()) {
      ...
      parsed = {
        ...
        ...(payload.conversationOrigin ? { conversationOrigin: payload.conversationOrigin } : {}),
        ...(payload.analyticsContext ? { analyticsContext: payload.analyticsContext } : {}),
        ...(payload.messageAnalytics ? { messageAnalytics: payload.messageAnalytics } : {}),
      };
    ...
    const chatResult = await this.chatEngineService.runChat({
      ...
      ...(parsed.conversationOrigin ? { conversationOrigin: parsed.conversationOrigin } : {}),

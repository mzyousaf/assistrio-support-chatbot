...
export interface EnrichedChunk {
  chunkId: string;
  documentId: string;
  /** KnowledgeBaseItem id when unified retrieval uses KB chunks (same as documentId today). */
  ...
  lexicalScore: number;
  /**
   * Primary unified retrieval match score (hybrid). Persisted on `Message.sources[].score`
   * via {@link normalizeAssistantMessageSourcesForPersistence} (maps `combinedScore` → `score`).
   */

...
import { isUsableEmailAvatarUrl } from '../email/email-avatar.util';
import { resolveAssistrioEmailLogoAssets } from '../email/email-assets.util';
...
export class WorkspaceInviteEmailService {
  ...
  async sendWorkspaceInviteEmail(input: SendWorkspaceInviteEmailInput): Promise<SendEmailResult> {
    ...
    const logo = resolveAssistrioEmailLogoAssets({
      logoPath1: this.configService.get<string>('emailLogoPath1'),
      logoPath2: this.configService.get<string>('emailLogoPath2'),
    });
    const { subject, html, text } = buildWorkspaceInviteEmailContent({
      ...
      supportUrl: this.configService.get<string>('supportUrl'),
      nodeEnv,
      logo,
    });
  ...
  private async resolveInviter(
    invitedByUserId: string,
  ): Promise<{ name: string | null; email: string | null; avatarUrl: string | null }> {
    if (!Types.ObjectId.isValid(invitedByUserId)) {
      return { name: null, email: null, avatarUrl: null };
    }
    const user = await this.userModel
      .findById(invitedByUserId)
      .select('email firstName lastName picture pictureOverride')
      .lean();
    ...
    const override = row.pictureOverride?.trim();
    const picture = override || row.picture?.trim() || null;
    const nodeEnv = this.configService.get<string>('nodeEnv');

    return {
      name: parts.length ? parts.join(' ') : null,
      email: row.email?.trim() || null,
      avatarUrl: picture && isUsableEmailAvatarUrl(picture, nodeEnv) ? picture : null,
    };
  }
}

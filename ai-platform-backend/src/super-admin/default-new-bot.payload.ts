/**
 * Default payload for a newly created bot (e.g. from POST /api/super-admin/bots/draft).
    ...
    documents: [
      {
        ...
        status: 'queued',
        fileSize: 2.85 * 1024 * 1024, // 2.85 MB
      },
      {
...
/** Map preset from API (e.g. support-agent) to schema enum (support). */
...
/**
 * Returns the flat payload suitable for Bot.create() when creating a new bot with defaults.
 */
export function getDefaultBotCreatePayload(
  ...
  return {
    ...
    knowledgeDescription: DEFAULT_NEW_BOT_PAYLOAD.knowledge.internalNotes,
    leadCapture: {
    ...
    chatUI: {
      ...
      liveIndicatorStyle: (ce.indicatorStyle === 'dot-only' ? 'dot-only' : 'label') as
        | 'label'
        | 'dot-only',
...
/**
 * Default template documents to add to a new bot (title + URL). Status is always queued.
 */

...
/**
 * Stream S3 object bytes without buffering the full object (Node.js runtime).
 * Caller must consume or destroy the stream to release the connection.
 */
...
/** Lightweight existence check for import/source pipelines (no body download). */
export async function headObjectExists(bucket: string, key: string): Promise<boolean> {
  if (!bucket?.trim() || !key?.trim()) return false;
  try {
    const client = getClient();
    await client.send(new HeadObjectCommand({ Bucket: bucket.trim(), Key: key.trim() }));
    return true;
  } catch {
    return false;
  }
...
/** Delete a private-bucket object (preview temp files, cleanup). */
export async function deletePrivateObject(bucket: string, key: string): Promise<void> {
  const client = getClient();

...
export class CustomerWorkspaceBillingCheckoutController {
  ...
  async checkoutPlan(
    ...
    return this.billingCheckoutService.createPlanCheckout(
      workspaceId,
      String(user._id),
      String(body?.planKey ?? ''),
  ...
  async checkoutAddon(
    @Req() req: RequestWithUser,
    @Param('workspaceId') workspaceId: string,
    @Body() body: { addonKey?: string; targetBotId?: string; billingInterval?: string },
  ) {
    const user = await this.assertWorkspaceOwner(req, workspaceId);
  ...
  @Post(':workspaceId/billing/checkout/auto-topup')
  async checkoutAutoTopUp(@Req() req: RequestWithUser, @Param('workspaceId') workspaceId: string) {
    const user = await this.assertWorkspaceOwner(req, workspaceId);
    return this.billingAiCreditsAutoTopUpService.beginEnableCheckout(workspaceId, String(user._id));
  }

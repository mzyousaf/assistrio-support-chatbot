...
describe('CustomerBotTopicsAnalyticsController', () => {
  ...
  it('throws NotFound when bot missing', async () => {
    ...
    await expect(
      ctrl.topicsAnalytics({ user: { _id: 'u1', role: 'member' } } as never, 'b1', {}),
  ...
  it('throws Forbidden when workspace denies bot access', async () => {
    ...
    await expect(
      ctrl.topicsAnalytics({ user: { _id: 'u1', role: 'member' } } as never, 'bot1', {}),
  ...
  it('calls analytics service when access granted', async () => {
    ...
    const res = await ctrl.topicsAnalytics(

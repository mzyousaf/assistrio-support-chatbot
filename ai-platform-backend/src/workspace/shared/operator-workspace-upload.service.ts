...
export class OperatorWorkspaceUploadService {
  ...
  private async ingestDocumentBatch(
    ...
    assertDocumentUploadBatchFileCountAtMost(fileParts.length, MAX_BOT_DOCUMENT_FILES_PER_REQUEST);
    ...
    for (let i = 0; i < fileParts.length; i++) {
      ...
      const trainSt = normalizeKnowledgeTrainingStatus(String((created as { status?: string }).status ?? ''));
      let ingestJobStatus: 'queued' | 'pending' = 'pending';
      let ingestJobId = '';
      const job = await this.ingestionService.createQueuedJob(botId, docId, { markTrainingQueued: false });

...
describe('CustomerBotSentimentAnalyticsController', () => {
  const getMock = jest.fn();
  const findOneMock = jest.fn();
  const canAccessMock = jest.fn();

  const ctrl = new CustomerBotSentimentAnalyticsController(
  ...
  it('throws NotFound when bot missing', async () => {
    ...
    await expect(
      ctrl.sentimentAnalytics({ user: { _id: 'u1', role: 'member' } } as never, 'b1', {}),
  ...
  it('throws Forbidden when workspace denies bot access', async () => {
    ...
    await expect(
      ctrl.sentimentAnalytics({ user: { _id: 'u1', role: 'member' } } as never, 'bot1', {}),
  ...
  it('calls analytics service when access granted', async () => {
    ...
    const res = await ctrl.sentimentAnalytics(
      { user: { _id: 'u1', role: 'member' } } as never,
      'bot1',
      { includePreview: 'false', granularity: 'week', sentiment: 'neutral' },
    );

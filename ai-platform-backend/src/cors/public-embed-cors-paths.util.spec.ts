...
describe('isPublicBrowserEmbedCorsPath', () => {
  describe('public (broad CORS) — representative', () => {
    ...
    it('iframe embed subtree (runtime)', () => {
      expect(isPublicBrowserEmbedCorsPath('/api/widget/iframe/init')).toBe(true);
      expect(isPublicBrowserEmbedCorsPath('/api/widget/iframe/chat')).toBe(true);
    });
  });
  ...
  describe('strict — must stay off public CORS', () => {
    ...
    it('hosted share preview API — strict CORS (not broad public embed)', () => {
      expect(isPublicBrowserEmbedCorsPath('/api/shared/bots/acme/init')).toBe(false);
      expect(isPublicBrowserEmbedCorsPath('/api/shared/bots/acme/chat')).toBe(false);
    });
  });

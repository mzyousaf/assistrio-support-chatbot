  ...
  async generateFaqs(botName: string, description: string): Promise<Array<{ question: string; answer: string }>> {
    ...
    try {
      const completion = await openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: `You generate 3–5 short FAQ pairs for an AI support assistant. Reply with a JSON array only, no markdown or extra text. Each item: { "question": "...", "answer": "..." }. Questions should be common user questions; answers 1–2 sentences.` },

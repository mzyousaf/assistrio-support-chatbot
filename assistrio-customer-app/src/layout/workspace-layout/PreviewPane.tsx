...
export type PreviewPaneProps = {
  ...
  /**
   * When true, the preview body uses `overflow-y-auto` so tall content (e.g. playground widget) scrolls
   * independently of the editor column. Default false: `overflow-y-hidden` for embeds that manage their own scroll.
   */
 ...
 * Default is `overflow-y-hidden` so embedded previews (e.g. widget) size to the column without a pane scrollbar.
 * Set {@link bodyScrollable} to true for an independent vertical scroll (e.g. playground widget column).
 */
export function PreviewPane({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'preview-surface flex min-h-0 flex-1 flex-col overflow-x-hidden',

...
export function Chat({
  ...
  const handleSend = useCallback(async () => {
    ...
    if (attach?.mode === "voice" && hasVoice) {
      sendResult = onSend(displayText, attach, files.length ? files : undefined);
    } else if (dictateInput) {
      sendResult = onSend(displayText, dictateInput, files.length ? files : undefined);
    } else {
      sendResult = onSend(displayText, undefined, files.length ? files : undefined);
    }
  ...
  const handleSuggested = useCallback(
    async (text: string) => {
      ...
      dictateTranscriptSegmentsRef.current = [];
      setInput(text);
      const r = await Promise.resolve(onSend(text));
      if (r === false) return;
      setInput("");
    },

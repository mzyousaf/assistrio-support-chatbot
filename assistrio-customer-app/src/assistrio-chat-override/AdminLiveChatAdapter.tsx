/**
 * Customer app only: fork of `chat-widget` `AdminLiveChatAdapter` (preview idle-session modal).
 * Bundler swaps this file via `assistrioCustomerChatOverridePlugin` in `vite.config.ts`. Keep
 * in sync when upgrading `@assistrio/chat-widget`.
 */
...
import type { ChatSpeechInputMeta } from "@acw/components/chat-ui/types";
import { mapSources } from "@acw/components/chat-ui";
import { ContainedLauncherPreview } from "@acw/components/ContainedLauncherPreview";
import { cx } from "@acw/components/chat-ui/utils";
import { useChatPanelBox } from "@acw/hooks/useChatPanelLayout";
import {
  PANEL_COLLAPSED_HEIGHT_PX,
  PANEL_COLLAPSED_WIDTH_PX,
  PANEL_EXPANDED_WIDTH_PX,
} from "@acw/lib/embedPanelConstraints";
import { containedLauncherPreviewBottomOutsetPx } from "@acw/lib/embedPanelConstraints";
import { launcherBubbleFromChatUI } from "@acw/lib/launcherBubbleFromChatUI";
import { apiFetch } from "@acw/lib/apiFetch";
import { fetchWithNetworkRetry } from "@acw/lib/fetchWithRetry";
import { buildClientAnalyticsContext } from "@acw/lib/clientAnalyticsContext";
import { buildWidgetMessageAnalytics } from "@acw/lib/widgetMessageAnalytics";
import { runtimeEmbedPost } from "@acw/lib/runtimeEmbedPost";
import { runtimeEmbedMultipartPost } from "@acw/lib/runtimeEmbedMultipartPost";
import { buildPlaygroundPreviewConversationOrigin } from "@acw/lib/playgroundPreviewConversationOrigin";
import { buildRuntimeWidgetConversationOrigin } from "@acw/lib/runtimeWidgetConversationOrigin";
...
/** Playground / authenticated preview: prompt to refresh the session after a long gap (matches customer dashboard). */
const PREVIEW_IDLE_SESSION_MS = 60 * 60 * 1000;

function maxThreadActivityMs(msgs: ChatUIMessage[]): number {
  let max = 0;
  for (const m of msgs) {
...
function shouldPromptPreviewIdleRefresh(msgs: ChatUIMessage[]): boolean {
  if (msgs.length === 0) return false;
  const last = maxThreadActivityMs(msgs);
  if (last <= 0) return false;
  return Date.now() - last > PREVIEW_IDLE_SESSION_MS;
}
...
export interface AdminLiveChatAdapterProps {
  ...
  /** Parent page URL when embedder passes `parentPageUrl` on the iframe src. */
...
export function AdminLiveChatAdapter({
  ...
  const skipIdleSessionCheckRef = useRef(false);
  const [idleSessionModalOpen, setIdleSessionModalOpen] = useState(false);
  const pendingIdleAfterRefreshRef = useRef<{
    text: string;
    speechInput?: ChatSpeechInputMeta;
    files?: File[];
    sendOptions?: { suggestionId?: string; dictationSessionCount?: number };
  } | null>(null);
  const pendingIdleRetryMessageIdRef = useRef<string | null>(null);
  ...
  const handleSelectRecent = useCallback(
    async (conversationId: string) => {
      ...
      if (mode === "preview") {
        setMessagesLoading(true);
        try {
          const fromApi = await fetchPreviewMessagesFromApi(conversationId);
    ...
    [
      ...
      chatVisitorId,
      welcomeMsg,
      fetchPreviewMessagesFromApi,
      previewVisitorStorageKey,
      runtimeSurface,
      sharedSlug,
      sharedContextPayload.shareToken,
      sharedContextPayload.pageUrl,
  ...
  const executeSend = useCallback(
    async (
      ...
      try {
        ...
        if (mode === "preview") {
          skipIdleSessionCheckRef.current = false;
        }
        setIsSending(false);
      }
    },
  ...
  const onSend = useCallback(
    async (
      ...
      if (
        mode === "preview" &&
        !skipIdleSessionCheckRef.current &&
        shouldPromptPreviewIdleRefresh(messages)
      ) {
        setIdleSessionModalOpen(true);
        pendingIdleAfterRefreshRef.current = {
          text: value,
          speechInput,
          files: fileArr.length ? fileArr : undefined,
          sendOptions: options,
        };
        return false;
      }
    ...
    [isSending, composerReadOnlyRuntime, executeSend, mode, messages],
  );
  ...
  const handleRetryMessage = useCallback(
    (messageId: string) => {
      if (
        ...
        setIdleSessionModalOpen(true);
        pendingIdleRetryMessageIdRef.current = messageId;
        return;
      }
    ...
    [executeSend, messages, mode],
  );

  const onIdleSessionModalDismiss = useCallback(() => {
    setIdleSessionModalOpen(false);
    pendingIdleAfterRefreshRef.current = null;
    pendingIdleRetryMessageIdRef.current = null;
  }, []);

  const onIdleSessionModalConfirm = useCallback(() => {
    setIdleSessionModalOpen(false);
    const pending = pendingIdleAfterRefreshRef.current;
    const retryId = pendingIdleRetryMessageIdRef.current;
    pendingIdleAfterRefreshRef.current = null;
    pendingIdleRetryMessageIdRef.current = null;

    const retryMsg = retryId
      ? messagesRef.current.find((m) => m.id === retryId && m.role === "user")
      : null;

    skipIdleSessionCheckRef.current = true;
    handleSessionStartNew();

    if (pending) {
      void onSend(pending.text, pending.speechInput, pending.files, pending.sendOptions);
    } else if (retryMsg) {
      const trimmed = retryMsg.content.trim();
      if (trimmed && !retryMsg.attachments?.length) {
        void onSend(trimmed, retryMsg.speechInput, undefined);
      }
    }
  }, [handleSessionStartNew, onSend]);

  useEffect(() => {
    if (!idleSessionModalOpen) return;
    const prev = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") onIdleSessionModalDismiss();
    };
  ...
  const idleSessionModal =
    mode === "preview" && idleSessionModalOpen && typeof document !== "undefined"
      ? createPortal(
        <div
          className="fixed inset-0 z-[2147483647] flex items-end justify-center p-0 sm:items-center sm:p-4"
          role="presentation"
        >
          <button
            type="button"
            ...
            aria-label="Close"
            onClick={onIdleSessionModalDismiss}
          />
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="assistrio-idle-session-title"
            className={cx(
              "relative z-[1] w-full max-w-md rounded-2xl border p-5 shadow-2xl outline-none",
              dark ? "border-gray-600/90 bg-gray-900 text-gray-200" : "border-gray-200 bg-white text-gray-800",
            )}
            onClick={(e) => e.stopPropagation()}
          >
            <h2
              id="assistrio-idle-session-title"
              className="text-base font-semibold leading-snug"
            >
              Chat inactive
            </h2>
            <p
              className={cx("mt-2 text-sm leading-relaxed", dark ? "text-gray-300" : "text-gray-600")}
            >
              There has been no activity in this chat for over an hour. Refresh the chat to continue. Your
              message will be sent in a new session.
            </p>
            <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
              <button
                ...
                onClick={onIdleSessionModalDismiss}
                className={cx(
                  "rounded-lg border px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1",
                  dark
                    ? "border-gray-600 text-gray-200 hover:bg-gray-800 focus-visible:ring-indigo-500/50 focus-visible:ring-offset-gray-900"
                    : "border-gray-300 text-gray-800 hover:bg-gray-50 focus-visible:ring-indigo-500/40",
                )}
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={onIdleSessionModalConfirm}
                className={cx(
                  "rounded-lg border border-transparent px-3 py-2 text-sm font-medium text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1",
                  dark
                    ? "bg-indigo-500 hover:bg-indigo-400 focus-visible:ring-indigo-300/50 focus-visible:ring-offset-gray-900"

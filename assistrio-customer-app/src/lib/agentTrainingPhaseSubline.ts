...
export function resolveAgentTrainingPhaseSubline(params: {
  ...
  if (phase === 'training_required' && params.pendingCount === 0) {
    return {
      ...
      text:
        density === 'compact'
          ? 'Saved changes aren’t queued yet — use Retrain agent.'
          : 'Your edits are saved but not in the training queue. Retrain agent queues pending items so embeddings update.',
    };
  }

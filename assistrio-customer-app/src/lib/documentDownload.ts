import { getCustomerBotDocumentDownloadUrl } from '../api/customerApi';
...
export function canonicalDocumentKbId(
  doc: Pick<CustomerWorkspaceDocument, '_id' | 'id' | 'knowledgeItemId'> | null | undefined,
): string {
  const k =
...
export async function openWorkspaceDocumentDownload(args: {
  ...
  const res = await getCustomerBotDocumentDownloadUrl(botId, docId);

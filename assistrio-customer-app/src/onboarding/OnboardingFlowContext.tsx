...
export function OnboardingFlowProvider({ children }: { children: ReactNode }) {
    ...
    (async () => {
      ...
      try {
        ...
        if (!loaded) {
          ...
          if (!cr.ok || cancelled) throw new Error(cr.ok ? 'Cancelled' : cr.error);
          const h = await loadDraft(cr.data.botId);
          if (!h || cancelled) throw new Error('Could not load the new draft.');
  ...
  const patchDraft = useCallback(
    async (body: Record<string, unknown>) => {
      ...
      const res = await patchCustomerBot(botId, body);

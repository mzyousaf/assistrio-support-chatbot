...
export function CaptureLeadsWorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const save = useCallback(async () => {
    ...
    if (leadEnabled && activeLeadFields.length === 0) {
      ...
      toastPlaygroundValidationWarning(
        'Lead capture incomplete',
        'Enable at least one field or turn off lead capture before saving.',
      );
      return;
    }
    ...
    if (!res.ok) {
      ...
      toastPlaygroundSectionSaveFailed('captureLeads', res.error);
      return;
    }
    toastPlaygroundSectionSaved('captureLeads');
    setDirty(false);
    await softReload();
  }, [
    bot,

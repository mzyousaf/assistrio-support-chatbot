...
import { getCustomerBotKnowledgePendingTrainingItems } from '../../../api/customerApi';
import { ASSISTRIO_WORKSPACE_BOT_REFRESH } from '@/lib/botSyncEvents';
import type {
  CustomerPendingTrainingItemsResponse,
  CustomerPendingTrainingItemDisplayStatus,
  CustomerPendingTrainingSectionType,
} from '../../../api/types';
...
export type NeedsTrainingModalProps = {
  ...
  prefetched?: CustomerPendingTrainingItemsResponse | null;
};
...
export function NeedsTrainingModal({
  ...
  const [data, setData] = useState<CustomerPendingTrainingItemsResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [expanded, setExpanded] = useState<CustomerPendingTrainingSectionType | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    setLoading(true);
    const res = await getCustomerBotKnowledgePendingTrainingItems(botId);
    setLoading(false);
    if (!res.ok) {
      setError(res.error);
      setData(null);
  ...
  const toggleSection = (type: CustomerPendingTrainingSectionType) => {

...
import type { CustomerKnowledgeOverviewResponse } from '../../../api/types';
...
export type KnowledgeSourcesModalBodyProps = {
  overview: CustomerKnowledgeOverviewResponse | null;
  loading: boolean;
        ...
        <ul className="m-0 list-none space-y-0 p-0">
          {rows.map((r) => (
            <li key={r.label} className="border-b border-slate-100 py-2.5 last:border-0">
              {botId ? (
                <Link
                  to={`/bots/${encodeURIComponent(botId)}/playground/knowledgebase/${encodeURIComponent(playgroundSegmentForKnowledgeBucket(r.bucket))}`}

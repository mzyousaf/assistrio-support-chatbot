...
import { patchCustomerBot, postCustomerKnowledgeSuggestionsSync } from '../../api/customerApi';
import {
  BEHAVIOR_PRESETS,
  ...
  const save = useCallback(async () => {
    ...
    if (!desc) {
      ...
      toastPlaygroundValidationWarning(
        ...
        'Add guidance under Personality so your agent knows how to respond.',
      );
    ...
    if (customCategoryMode) {
      ...
      if (selectedCategories.length === 0) {
        ...
        toastPlaygroundValidationWarning(
          'Category required',
          'Pick at least one category that describes your agent.',
        );
        setActiveSubnav('personality');
        return;
      }
    ...
    const sugRes = await postCustomerKnowledgeSuggestionsSync(botId, examplePatch);
    if (!sugRes.ok) {
      setSaving(false);

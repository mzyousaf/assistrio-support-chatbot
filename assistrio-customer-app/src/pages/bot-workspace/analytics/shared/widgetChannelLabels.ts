import type { CustomerChatsAnalyticsStartedFromKey } from '@/api/types';

/** Customer-facing label for each analytics `startedFrom` / widget channel key. */
export const WIDGET_CHANNEL_LABELS: Record<CustomerChatsAnalyticsStartedFromKey, string> = {
...
export function isWidgetChannelKey(id: string): id is CustomerChatsAnalyticsStartedFromKey {
  return WIDGET_CHANNEL_LABEL_SET.has(id);
}

/** Readable widget channel name for filters, tables, badges, and insights. */
export function widgetStartedFromUiLabel(id: CustomerChatsAnalyticsStartedFromKey | string): string {

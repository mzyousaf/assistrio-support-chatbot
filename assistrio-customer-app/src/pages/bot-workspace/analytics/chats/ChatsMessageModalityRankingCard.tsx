...
import type { ChatsMessageModalitySeriesId } from './chatsMessageModalityChartTheme';

export type ChatsMessageModalityRankingRow = {
  id: ChatsMessageModalitySeriesId;
  label: string;
  color: string;
  count: number;
};
...
type Props = {
  rows: ChatsMessageModalityRankingRow[];
  hiddenSeriesIds: string[];
  onToggleSeries: (id: ChatsMessageModalitySeriesId) => void;
};

export function ChatsMessageModalityRankingCard({ rows, hiddenSeriesIds, onToggleSeries }: Props) {
  if (!rows.length) {
    return <p className="m-0 px-2 py-4 text-center text-sm text-slate-500">No data for this range.</p>;
  }

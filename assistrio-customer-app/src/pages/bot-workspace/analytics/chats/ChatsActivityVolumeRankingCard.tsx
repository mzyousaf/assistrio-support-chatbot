...
import type { ChatsActivitySeriesId } from './chatsActivityChartTheme';

export type ChatsVolumeRankingRow = {
  id: ChatsActivitySeriesId;
  label: string;
  color: string;
  count: number;
};
...
type Props = {
  rows: ChatsVolumeRankingRow[];
  hiddenSeriesIds: string[];
  onToggleSeries: (id: ChatsActivitySeriesId) => void;
};

export function ChatsActivityVolumeRankingCard({ rows, hiddenSeriesIds, onToggleSeries }: Props) {
  if (!rows.length) {
        ...
        return (
          <li key={row.id}>
            <button
              ...
              className={cn(
                'flex w-full min-w-0 min-h-[2.5rem] flex-col rounded-lg border border-transparent px-2 py-2 text-left transition-colors duration-200',

    ...
    <div className="w-full min-w-0 border-t border-slate-200/80 pt-5">
      ...
      >
        <p className={cn(ws.workspaceEditorSubsectionTitle, 'mb-2 text-slate-900')}>
          {embedInstallMode === 'chat-widget' ? 'Chat widget Instructiuon' : 'Iframe embed'}
        </p>
        {embedInstallMode === 'chat-widget' ? (
          ...
          <ol className={embedStepClass}>
            <li>
              Copy the placeholder from <span className="font-semibold text-slate-900">Install code</span> on the right.
            </li>
            <li>
              Set the iframe <code className={codeChip}>src</code> (or your proxy URL). The page origin must still match an allowed website.
            </li>
            <li>
              For the full launcher experience, switch <span className="font-semibold text-slate-900">Embed type</span> to Chat widget.
            </li>
          </ol>
        )}

...
const BASIS_LABEL: Record<string, string> = {
  ...
  attachment_message: 'Attachment-only',
...
export function buildConversationCreditBreakdown(messages: CustomerConversationMessage[] | null | undefined): ConversationCreditBreakdownPayload {
  ...
  for (let i = 0; i < userRows.length; i += 1) {
    ...
    const basisKey = normalizeCreditBasis(reasonRaw) ?? 'unknown_message';
    const kindLabel = chargedKindDisplay(reasonRaw);
    ...
    if (breakdown) {
      for (const row of breakdown) {
        const key = row.key;
        const curC = componentMap.get(key);
        ...
        const label = row.label || BASIS_LABEL[row.key] || row.key;
        if (!curC) componentMap.set(key, { key, label, billedUnits: 0, creditsUsed: 0 });
        const slot = componentMap.get(key)!;
        slot.label = label;

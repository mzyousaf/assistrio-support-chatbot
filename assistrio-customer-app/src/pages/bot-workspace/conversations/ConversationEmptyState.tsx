...
export function ConversationEmptyState({ hasActiveFilters, listError, onRetry }: Props) {
  if (listError) {
    return (
      <div className="flex flex-col items-center justify-center px-3 py-8 text-center" role="alert">
  ...
  const title = hasActiveFilters ? 'No conversations match these filters' : 'No conversations yet';
  const body = hasActiveFilters
    ? 'Try clearing filters or widening the date range.'
    : 'When visitors chat with your assistant, threads will appear here.';

  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-3 py-8 text-center" role="status">
      <div

...
const MAIN_TOPIC_ORDER = new Map<string, number>(CUSTOMER_TOPICS_ANALYTICS_MAIN_TOPIC_IDS.map((id, i) => [id, i]));

const SENTIMENT_ORDER = new Map<string, number>([...SENTIMENT_STACK_ORDER].map((id, i) => [id, i]));

export function sortConversationTaxonomyTopicKeys(keys: Iterable<string>): CustomerTopicsAnalyticsTopicId[] {
  const uniq = [
    ...new Set(
      [...keys]
        .map((x) => x.trim())
        .filter(Boolean)
        .filter((id): id is CustomerTopicsAnalyticsTopicId => MAIN_TOPIC_ORDER.has(id)),
    ),
  ].sort((a, b) => MAIN_TOPIC_ORDER.get(a)! - MAIN_TOPIC_ORDER.get(b)!);
  return uniq;
}

export function sortConversationSentimentKeys(keys: Iterable<string>): CustomerSentimentLabelId[] {
  const uniq = [
    ...new Set(
      [...keys]
        .map((x) => x.trim().toLowerCase())
        .filter(Boolean)
        .filter((id): id is CustomerSentimentLabelId => SENTIMENT_ORDER.has(id)),
    ),
  ].sort((a, b) => SENTIMENT_ORDER.get(a)! - SENTIMENT_ORDER.get(b)!);
  return uniq;
}

function topicsFromCommaParam(raw: string | undefined | null): CustomerTopicsAnalyticsTopicId[] {
  if (!raw?.trim()) return [];
  return sortConversationTaxonomyTopicKeys(raw.split(','));
}

function sentimentsFromCommaParam(raw: string | undefined | null): CustomerSentimentLabelId[] {
  if (!raw?.trim()) return [];
...
export function sortConversationWidgetChannelKeys(keys: CustomerChatsAnalyticsStartedFromKey[]): CustomerChatsAnalyticsStartedFromKey[] {
...
export type ConversationFiltersDraft = {
  ...
  countryCode: string;
  primaryTopicKeys: CustomerTopicsAnalyticsTopicId[];
  /** Stored on chats as `conversationTopics.topicLabels` (main taxonomy). */
  secondaryTopicKeys: CustomerTopicsAnalyticsTopicId[];
  sentimentKeys: CustomerSentimentLabelId[];
};
...
export function parseConversationStartedFromParam(
  ...
  ].filter((k): k is CustomerChatsAnalyticsStartedFromKey => VALID_STARTED_FROM.has(k));
  return keys;
}
...
export function conversationAppliedFiltersSummary(
  applied: CustomerBotConversationsListParams,
...
export function removeAppliedConversationFilterByChipId(
  ...
  const next: CustomerBotConversationsListParams = { ...applied };
  switch (chipId) {
    case 'date':

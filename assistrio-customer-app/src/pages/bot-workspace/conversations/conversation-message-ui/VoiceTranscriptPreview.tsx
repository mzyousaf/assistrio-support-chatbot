...
function fitTailWordsToLineWidth(tail: string, maxWidth: number, host: HTMLElement): string {
  ...
  const div = document.createElement('div');
  div.setAttribute('aria-hidden', 'true');
  div.className = cn('inline-block text-left text-[13px] leading-snug whitespace-nowrap', transcriptMuted());
...
function TranscriptThirdLine({
  ...
  dark: boolean;
  onSeeMore?: () => void;
  measureHostRef: RefObject<HTMLDivElement | null>;
}) {
...
export function VoiceTranscriptPreview({
  ...
  if (split == null) {
    return (
      <div ref={hostRef} className="w-full min-w-0 max-w-full text-left">
        <p className={cn(textBase(), '!text-left', 'line-clamp-2')}>{t}</p>
      </div>
    );
  }
  ...
  if (!showThirdRow) {
    return (
      <div ref={hostRef} className="w-full min-w-0 max-w-full text-left">
        <p className={cn(textBase(), '!text-left')}>{t}</p>
      </div>
    );
  }
  ...
  return (
    <div ref={hostRef} className="w-full min-w-0 max-w-full text-left">
      {head ? (
        <p className={cn(textBase(), 'm-0 !text-left whitespace-pre-wrap break-words [overflow-wrap:anywhere]')}>
          {head}
        </p>
      ) : null}
      <TranscriptThirdLine tail={tail} dark={dark} onSeeMore={onSeeMore} measureHostRef={hostRef} />
    </div>
  );
}

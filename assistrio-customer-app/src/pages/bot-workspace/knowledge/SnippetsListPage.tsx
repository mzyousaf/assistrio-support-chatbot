...
import {
  getCustomerKnowledgeSnippetImportCsvSample,
  patchCustomerKnowledgeItemUseInReplies,
  postCustomerKnowledgeSnippet,
  postCustomerKnowledgeSnippetImportCsv,
...
export function SnippetsListPage() {
  ...
  async function downloadSnippetSampleCsv() {
    if (!botId) return;
    const res = await getCustomerKnowledgeSnippetImportCsvSample(botId);
    if (!res.ok) {
  ...
  async function importSnippetCsv(file: File | undefined) {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await postCustomerKnowledgeSnippetImportCsv(botId, fd);
      if (!res.ok) {
        const details =
  ...
  async function persistSnippetForAdd(title: string, snippet: string, successToast: string): Promise<boolean> {
    ...
    setAddSaving(true);
    try {
      const res = await postCustomerKnowledgeSnippet(botId, {

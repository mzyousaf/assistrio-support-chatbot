...
import { patchCustomerKnowledgeFaq } from '../../../api/customerApi';
...
export function QaEditPage() {
  ...
  async function persistFaqs(
    ...
    try {
      ...
      const res = await patchCustomerKnowledgeFaq(botId, numIndex, payload as Record<string, unknown>);
      if (tryHandleCustomerResourceGone(navigate, res, `${base}/faqs`)) {
        return { committed: false };
      }
      if (!res.ok) {
